...
fn main() -> eyre::Result<()> {
    ...
    for (colname, col_data) in args.column.iter().zip(&mut data) {
        ...
        println!("    count: {}", stats.num);
        if stats.num_filtered > 0 {
            println!("    filtered: {} (total: {})", stats.num_filtered, stats.num + stats.num_filtered);
        }

        // quartiles are computed by from_unsorted(), which isn't purely online.
        println!("    Q1: {}", stats.q1.unwrap());
        println!("    median: {}", stats.median.unwrap());
        println!("    Q3: {}", stats.q3.unwrap());

        println!("    min: {} at index: {}", stats.min, stats.min_index);
        println!("    max: {} at index: {}", stats.max, stats.max_index);
